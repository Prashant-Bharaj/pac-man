...
**2. Per-frame maze redraw (`src/renderer.py`)** — the maze never changes during a level, but `_draw_maze` was issuing ~3 000 `pygame.draw.rect` calls per frame (59×59 expanded grid at level 9). The renderer now pre-renders the maze to a `pygame.Surface` once on level load and blits that surface each frame instead.
...
```
main.py                     # Entry point — parses args, loads config, starts Game
│
